...
IOTCL_SyncResponse* iotc_wiced_discover(const char *env, const char *cpid,
        const char* duid) {
    ...
    if (strlen(data_buff) > 0) {
        ...
        if (strlen(data_buff) > 0) {
            // char * json_start = strstr(data_buff, "{");
            IOTCL_SyncResponse * sr = IOTCL_DiscoveryParseSyncResponse(
                    data_buff);
            if (NULL == sr) {
                WPRINT_LIB_ERROR(("Error: NULL Sync Response\n"));
            }
            return sr;
        }
    }
...
void synchronous_rest_call(const char *host, const char *path,
    ...
    wiced_hostname_lookup(host, &ip_address, DNS_TIMEOUT_MS,
            WICED_STA_INTERFACE);
    WPRINT_LIB_INFO(
            ("%s is at %u.%u.%u.%u\n", host, (uint8_t)(GET_IPV4_ADDRESS(ip_address) >> 24), (uint8_t)(GET_IPV4_ADDRESS(ip_address) >> 16), (uint8_t)(GET_IPV4_ADDRESS(ip_address) >> 8), (uint8_t)(GET_IPV4_ADDRESS(ip_address) >> 0)));
