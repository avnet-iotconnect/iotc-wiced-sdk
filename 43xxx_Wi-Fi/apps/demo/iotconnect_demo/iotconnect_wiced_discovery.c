//
// Created by nmarkovi on 1/12/21.
//
...
static char data_buff[500];
static http_client_t client;
static http_request_t request;
static wiced_semaphore_t semaphore;

// forward declarations -----------
static void synchronous_rest_call(const char *host, const char *path, const char *post_data);

static void event_handler(http_client_t *client, http_event_t event, http_response_t *response);

static void set_header_field(http_header_field_t *field, const char * name, const char * value);
...
void iotc_wiced_discovery_init(void) {
    ...
    if (result != WICED_SUCCESS) {
        WPRINT_APP_INFO(("Error: Failed to initialize HTTP client: %u\n", result));
        return;
    }
...
IOTCL_SyncResponse* iotc_wiced_discover(const char *env, const char *cpid, const char* duid) {
    ...
    if (strlen(data_buff) > 0) {
        ...
        IOTCL_DiscoveryResponse * dr = IOTCL_DiscoveryParseDiscoveryResponse(json_start);
        WPRINT_APP_INFO(("Agent URL: %s\n", dr->url));
        ...
        if (strlen(data_buff) > 0) {
            // char * json_start = strstr(data_buff, "{");
            IOTCL_SyncResponse * sr = IOTCL_DiscoveryParseSyncResponse(data_buff);
            if (NULL == sr) {
                WPRINT_APP_ERROR(("Error: NULL Sync Response\n"));
            }
            return sr;
        }
    }
...
void synchronous_rest_call(const char *host, const char *path, const char* post_data) {
    ...
    wiced_hostname_lookup(host, &ip_address, DNS_TIMEOUT_MS, WICED_STA_INTERFACE);
    ...
    if ((result = http_client_connect(&client, (const wiced_ip_address_t *) &ip_address, PORT, HTTP_USE_TLS,
    ...
    if (post_data) {
        char content_len_buffer[4];
        sprintf(content_len_buffer, "%u", strlen(post_data));
        set_header_field(&header[2], HTTP_HEADER_CONTENT_TYPE, "application/json");
        set_header_field(&header[3], HTTP_HEADER_CONTENT_LENGTH, content_len_buffer);\
    ...
    if (post_data) {
        http_request_write(&request, (const uint8_t*)post_data, strlen(post_data));
    ...
    if (wiced_rtos_get_semaphore(&semaphore, 20000) != WICED_SUCCESS) {
        WPRINT_APP_INFO(("Error: timed out after 20s waiting for HTTP communication to complete\n"));
    }
    //http_request_deinit(&request);

    WPRINT_APP_INFO(("Disconnecting.\n"));
...
static void event_handler(http_client_t *client, http_event_t event, http_response_t *response) {
    switch (event) {
        case HTTP_DISCONNECTED: {
            WPRINT_APP_INFO(("HTTP Disconnected\n"));
        ...
        case HTTP_DATA_RECEIVED: {
            if (response->request == &request) {
                ...
                WPRINT_APP_INFO(("Payload Information for response:\n"));
                ...
                if (response->remaining_length == 0) {
                    WPRINT_APP_INFO(("Received total payload data for response\n"));
                }
            }
            break;
        }
...
static void set_header_field(http_header_field_t *header_item, const char * name, const char * value) {
